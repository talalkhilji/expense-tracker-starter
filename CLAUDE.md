...
`src/App.jsx` (mounted by `src/main.jsx`) is the top-level component. It owns the `transactions` array (`useState`) and the hardcoded `categories` list, and composes three child components. There is no routing and no external state management — state is local `useState`, split across components by responsibility rather than centralized in `App`.
